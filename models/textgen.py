...
class SAERTextRNN(TextRNN):
    ...
    def decode(self, input_seq, hidden, data=None, input_dict=None):
        ...
        # if self.use_attn:
        #     embedded_mem = self.word_ebd(word_mem)
        #     embedded_mem = self.word_ebd_dropout(embedded_mem)

        #     ctx, _ = self.mem_attn(output.transpose(0, 1), embedded_mem)
        #     ctx = ctx.transpose(0, 1)

        #     output = torch.cat((output, ctx), 2)
...
class WordMLP(nn.Module):
    ...
    def __init__(
        ...
        self.output = nn.Linear(i_size, n_words)
    ...
    def forward(self, input_var):
        ...
        for layer in self.layers:
            ...
            layer_outputs.append(output_var)

        output_var = self.output(output_var)
        return output_var
