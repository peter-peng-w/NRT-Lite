...
from loss import mask_nll_loss, mask_ce_loss, bpr_loss, lambda_rank_loss, rank_hinge_loss, RLMCRankingLoss, MRTRankingLoss, review_loss
...
import numpy as np
from constants import RR_LOSS_WEIGHT, WG_LOSS_WEIGHT, TG_LOSS_WEIGHT, REG_WEIGHT
...
class NRTLiteTrainer(object):
    """implements the training algorithm for the model."""
    ...
    def __init__(self, model):
        ...
        self.loss_criterion = (
            ...
            RR_LOSS_WEIGHT * nn.MSELoss()(r_hat, r) + WG_LOSS_WEIGHT * review_loss(c_hat, c) + TG_LOSS_WEIGHT * nn.NLLLoss()(s_hat, s)
        )
    ...
    def train(self, train_iter, n_epochs):
        ...
        for epoch_i in range(n_epochs):
            for batch in tqdm(
                    train_iter,
                    desc="Epochs: {} / {}, Loss: {}".format(epoch_i, n_epochs, losses[-1] if len(losses) > 0 else np.inf)):
                ...
                loss = self.loss_criterion(
                    ratings_batch, regression_result,
