...
#     def __init__(self, model, voc, max_length=15, ranker=None, topk=5, rollout=5):
#         super().__init__()
#         self.model = model
#         self.voc = voc
#         self.max_length = max_length
...
#         self.sampler = SearchDecoder(model, voc, greedy=False)

#     def mc_search(self, hidden, word_idxes, prefix_words, scores, data, input_dict, search_length):
#         '''
...
#         action_values = torch.zeros(word_idxes.size(), device=word_idxes.device)

#         # Monte Carlo Search with Rollout policy
#         for i in range(self.rollout):
#             for action_idx in range(self.topk):
#                 # (batch, k) -> (batch)
#                 actions = word_idxes[:, action_idx]

#                 sampled_result = self.sampler.decode(hidden, actions, max_length=search_length, data=data, input_dict=input_dict)
...
#                 action_values[:, action_idx] += mse_loss(sampled_scores, scores, reduction='none')

#         action_values /= self.rollout
...
#             # (1, batch, n_words) -> (batch, n_words)
#             word_ll = output.squeeze(0)

#             # (batch, k)
#             word_ll, word_idxes = word_ll.topk(self.topk)

#             # rollout_idx = output_dict.rate_gates > 0.5
#             if i < 5: # rollout first 5 words
#                 seach_length = max_length - i - 1
#                 prefix_words = torch.stack(words, dim=0) if words else torch.tensor([], dtype=torch.long, device=word_idxes.device)

#                 action_values = self.mc_search(hidden, word_idxes, prefix_words, scores, data, input_dict, seach_length)

#                 action_idx = action_values.argmin(1, keepdim=True)
#             else:
#                 action_idx = torch.multinomial(word_ll.exp(), 1)

#             prob_var = word_ll.gather(-1, action_idx).view(-1).exp()
#             word_var = word_idxes.gather(-1, action_idx).view(-1)
...
class MCSearchDecoder(AbstractSearchDecoder):
    ...
    def decode(self, hidden, start_var, data=None, input_dict=None, max_length=None):
        ...
        feature_counts = torch.zeros(batch_size, len(self.voc), dtype=torch.long, device=start_var.device)
