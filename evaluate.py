...
def load_ndcg(ndcg_path):
    ...
    with open(ndcg_path) as f:
        ...
        ndcg_user_items = {int(uid): items for uid, items in ndcg_user_items.items()}
    return ndcg_user_items


def test_review_ndcg(model, voc, dev_data=None, ndcg_user_items=None, ndcg_path=None, searcher=None, pred_rates=False):
    ...
    for i in range(0, len(user_items), batch_size):
        ...
        if not searcher:
            ...
            output, _ = model(search_result.words, rvw_lens=search_result.rvw_lens)
...
def test_rate_ndcg(test_data, model, ndcg_user_items=None, ndcg_path=None, k=[], builder=basic_builder, batch_size=BATCH_SIZE):
    ...
    for i in range(0, len(user_items), batch_size):
        ...
        pure_ranked_scores = ranked_scores[ranked_scores != -1].view(n_grps, -1)
        ranked_scores[ranked_scores == -1] = 0

        if config.RANKING_RATING_THRESHOLD:
            ranked_scores[ranked_scores < config.RANKING_RATING_THRESHOLD] = 0
            ranked_scores[ranked_scores >= config.RANKING_RATING_THRESHOLD] = 1

            pure_ranked_scores[pure_ranked_scores < config.RANKING_RATING_THRESHOLD] = 0
            pure_ranked_scores[pure_ranked_scores >= config.RANKING_RATING_THRESHOLD] = 1
